...
void buduj_drzewo(list<string> database[n], int xDecisive)
{

    if()//jesli gain ratio (X,T) == 0 to wartosc_atrybutu -> decyzja
    else//dalszy rozrost drzewa na podstawie wyznaczonego nowego atrybutu decyzyjnego
...
int main()
{
    ...
    */
    ...
    cout<<"\n\n\nAtrybut: "<<xDecisive+1<<endl;
    //tutaj wywolanie rekurencyjne
    for(list<elem>::iterator it1 = records[xDecisive].begin();it1 != records[xDecisive].end();it1++)
    {
        for(list<string>::iterator it = database[xDecisive].begin();it != database[xDecisive].end();it++)
            if(*it == it1->name)
            {
